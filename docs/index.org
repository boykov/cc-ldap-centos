...
В конфигурации *5/2.3* приходится использовать параметр
*enableshadow*, поскольку эта функциональность отключена по
умолчанию. Отдельно укажем параметр *sudoers_base* и параметры
*binddn* и *bindpw* в настройках для *pam_ldap*.
...
Для конфигурации *6/2.4* приходится отдельно прописывать параметр
*sudoers* для *nsswitch.conf*, а в конфигурации *5/2.3* он уже
появляется после установки пакетов, еще до запуска *authconfig*. Для
настройки *sudo* дополнительно указываются параметры *uri* и *base*,
поскольку файл теперь другой --- *sudo-ldap.conf*. А параметры
...
*passwd* --- несущественные изменения, связанные с опцией
*enableshadow* и с добавлением ldap; *system-auth-ac* и
*system-auth* это один и тот же файл; файл *authconfig* просто
содержит все опции, указанные при конфигурировании.

Для конфигурации *6/2.4*, кроме файлов *nsswitch.conf*,
...
- *sudo-ldap.conf* --- ldap-специфичная конфигурация для *sudo*. В
  случае конфигурации *5/2.3* для этого использовался файл
  */etc/ldap.conf*.
...
*/etc/ldap.conf*. Параметр *host* не рекомендуется к использованию,
...
*nsswitch.conf*. Для баз данных *passwd*, *shadow*, *group*,
...
*** Опции, связанные с system-auth для *5/2.3*
:PROPERTIES:
...
#+caption: Опции, выставляемые в файлах для *5/2.3* (б)
...
#+caption: Опции, выставляемые в файлах для *6/2.4* (а)
#+INCLUDE: /home/eab/git/cc/cc-ldap-centos/gen/client6.diff src diff :lines "1-38"

*** Опции, связанные с system-auth для *6/2.4*
:PROPERTIES:
:ID:       5bedb39f-f4fd-416b-9c11-ebba1119ac1a
:END:

#+caption: Опции, выставляемые в файлах для *6/2.4* (б)
...
Information Tree) *cn=config* и заканчивая листьями этого дерева ---
...
*people*. Используем учетную запись *authenticator* для ограничения
...
#+caption: *sudo.ldif*: настройка организационной единицы *sudoers*, добавление группы *wheel*
#+name: sudo-ldif
