...
<<schema2ldif>>
#+end_src
#+caption: Файл *startup-config.ldif*
#+INCLUDE: ../ldap-server/startup-config.ldif src ldif
...
Например, для конфигурации *5/2.3* требуется добавить настройки *frontend* базы
#+begin_src sh :exports code :noweb yes
<<add-front5>>
#+end_src
а для *6/2.4* эти настройки добавляются автоматически из исходного кода *OpenLDAP*.

#+caption: Файл *front.ldif*
#+name: front-ldif
#+INCLUDE: ../ldap-server/front.ldif src ldif
...
Добавим минимально необходимые настройки для *monitor* и *bdb*
#+caption: Начальное конфигурирование схемы БД LDAP для *5/2.3* и *6/2.4*
#+begin_src sh :exports code :noweb yes
<<run-modify>>
#+end_src

#+caption: Файл *slapd.ldif*
#+name: slapd-ldif
#+INCLUDE: ../ldap-server/slapd.ldif src ldif
...
#+INCLUDE: /home/eab/git/cc/cc-ldap-centos/schema/custom-ShadowAccount.xml src xml :exports (when (eq org-export-current-backend 'odt) "none")
#+HTML: </div>
