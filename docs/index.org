...
Чтобы иметь доступ к серверу LDAP для конфигурирования схемы БД LDAP, внесем необходимые изменения: пропишем пароль менеджера схемы *cn=Manager,cn=config*. Для обеих конфигураций выполним следующие действия
...
** {{{fa(star)}}} Возможности LDAP
:PROPERTIES:
:CUSTOM_ID: ldap-possibilities
:ID:       4f9ddacc-3908-4cd1-b091-3f00c31aa8a7
:END:

Возможности протокола LDAP определяются схемами, которые можно подключить с помощью директивы *include*. Помня об основной цели, рассмотрим типичные задачи, которые можно решить с помощью протокола LDAP. В первую очередь нас интересуют уже готовые схемы, входящие в дистрибутив *OpenLDAP* или разработанные сторонними поставщиками.
...
- [[https://www.fusiondirectory.org/service/][FusionDirectory]]
...
* {{{fa(star-half-o)}}} Процесс добавления нового пользователя в систему
:PROPERTIES:
:CUSTOM_ID: new-user
...
** {{{fa(star)}}} Добавление пользователя с помощью GUI Web интерфейса
:PROPERTIES:
