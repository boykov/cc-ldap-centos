...
| *Шлюз, docker хост* | 172.17.0.1           |                            |            |
| *Серверы каталогов* |                      |                            |            |
|                     | cc-ldap-server-ldif5 | *Centos 5*, *OpenLDAP 2.3* | slapd.ldif |
|                     | cc-ldap-server-ldif6 | *Centos 6*, *OpenLDAP 2.4* | slapd.ldif |
|                     | cc-ldap-server-conf5 | *Centos 5*, *OpenLDAP 2.3* | slapd.conf |
|                     | cc-ldap-server-conf6 | *Centos 6*, *OpenLDAP 2.4* | slapd.conf |
| *Клиентские хосты*  |                      |                            |            |
|                     | cc-ldap-client5      | *Centos 5*, *OpenLDAP 2.3* |            |
|                     | cc-ldap-client6      | *Centos 6*, *OpenLDAP 2.4* |            |

IP-адреса в подсети 172.17.0.x определяются на основе имен контейнеров.
...
Конфигурация *5/2.3* по умолчанию настраивается файлом *slapd.conf*,
при этом каталог *slapd.d* заранее отсутствует и создается при первом
конфигурировании. Такой подход считается устаревшим, [[http://www.openldap.org/doc/admin24/guide.html#Converting%20old%20style%20{{slapd.conf}}%285%29%20file%20to%20{{cn=config}}%20format][рекомендуется]]
использовать *ldif* формат. Конфигурация *6/2.4* поставляется с тремя
вариантами базовых настроек, осуществляемых через файлы
*slapd.conf.obsolete*, *slapd.ldif.example* и, доступным по умолчанию,
предварительно сформированным каталогом *slapd.d*. Для обеих конфигураций
будем создавать каталог *slapd.d* с нуля, используя *ldif* формат (то
есть второй вариант - *slapd.ldif.example*), не применяя *.conf* файлы
или готовый каталог *slapd.d*.

Общая схема БД LDAP, в том числе, содержит раздел *cn=schema*, куда подключаются дополнительные специфические схемы.  Добавим необходимые схемы (не включенные в поставку *OpenLDAP*), для *5/2.3*
...
#+end_src
#+caption: Добавление схем для *5/2.3*
#+begin_src sh :exports code :noweb yes
<<schema-prepare5>>
#+end_src
и для *6/2.4*
#+caption: Добавление схем для *6/2.4*
#+begin_src sh :exports code :noweb yes
<<schema-prepare6>>
#+end_src
...
запись для всего содержимого.  Используем "оффлайновую" утилиту
*slapadd* для создания минимума конфигурационных настроек, позволяющих
запустить сервер.
...
<<schema2ldif>>
#+end_src
#+caption: *startup-config.ldif*: формирование корневой записи DIT и контейнера для наборов схем данных
#+INCLUDE: ../schema/startup-config.ldif src ldif
...
<<run-slapd-start6>>
#+end_src
получаем сервисы, отличающиеся только настройками схемы, содержащимися в исходных кодах разных версий *OpenLDAP*.
Например, для конфигурации *5/2.3* требуется добавить настройки *frontend* базы, а для *6/2.4* эти настройки добавляются автоматически из исходного кода *OpenLDAP*, поскольку версия *2.4* официально поддерживает конфигурирование с помощью *slapadd* и *ldif* формат. Для версии *2.3* официально поддерживаемым является способ конфигурирования через *conf* файл. Кроме того, добавим минимальные настройки для *monitor* и настроим базу *hdb*. Все дальнейшие изменения уже осуществляются с помощью "онлайновых" утилит *ldapadd* и *ldapmodify*.
...
#+caption: *slapd.ldif*: конфигурирование базы данных *hdb* и включение мониторинга баз данных
#+name: slapd-ldif
...
** Предпочтительный способ настройки схемы БД LDAP
:PROPERTIES:
:CUSTOM_ID: configure-slapd-conf
:ID:       fd3e3e26-d416-4cc5-9dfa-554caeaf7830
:END:

В предыдущем разделе продемонстрирован способ настройки схемы,
используя только *ldif* формат. Однако, на практике может оказаться
предпочтительным не подготавливать тщательно нужную начальную
конфигурацию с помощью файлов *startup-config.ldif* и *slapd.ldif*, а
воспользоваться автоматической генерацией каталога *slapd.d* сервером
*OpenLDAP* на основе конфигурационных файлов. При этом, в схему
прописывается множество дополнительных параметров.
...
Использование только *ldif* формата позволяет единообразно конфигурировать схему *LDAP*, начиная с корневой записи DIT (Directory Information Tree) *cn=config* и заканчивая листями этого дерева - записями, содержащими нужную информацию:
#+begin_src sh :exports results :results raw
grep -i -h -e  "^#+caption.*\\.ldif" index.org | sed "s/#+caption://g" | sed "s/:/|/g" | sed "s/^/|/"
#+end_src

#+RESULTS:
| *startup-config.ldif* | формирование корневой записи DIT и контейнера для наборов схем данных |
| *slapd.ldif*          | конфигурирование базы данных *hdb* и включение мониторинга баз данных |
