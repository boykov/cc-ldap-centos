...
системной схемы данных, которые вкомилированы в *ldap*
(*slapd*). Сюда же подключаются дополнительные специфические схемы
данных. Подготовим необходимые файлы (не включенные в поставку
*OpenLDAP*), для *5/2.3*
...
схем данных/, означающим файл с расширением *schema* или *ldif*,
содержащий "низкоуровневое" описание классов, атрибутов, синтаксисов
атрибута и правил соответствия. Также, оно используется в выражении
/схема каталога LDAP/, т.е. для "высокоуровнего" описания
содержания, структуры и ограничений целостности каталога LDAP, по
аналогии с выражением /схема базы данных/ применимым для реляционных
баз данных. Кроме того, /"каталог LDAP"/ использует /базу данных
*HDB*/ в качестве конечного хранилища данных.
#+end_note
...
сформированную директорию *slapd.d* без *conf*-файла, осуществляется при
...
формата позволяет единообразно конфигурировать схему каталога LDAP, начиная
...
и атрибутов схемы каталога LDAP.
...
схема каталога LDAP --- должна обеспечивать поддержку данной
