...
** Выбор способа настройки схемы БД LDAP
:PROPERTIES:
:CUSTOM_ID: choose-setup
:ID:       cb0ed9d0-14cc-4969-897d-a398e2f1ea65
:END:

Конфигурация *5/2.3* по умолчанию настраивается только файлом
*slapd.conf*, предварительно сформированный каталог *slapd.d* в
формате *ldif* отсутствует. Такой подход считается устаревшим,
[[http://www.openldap.org/doc/admin24/guide.html#Converting%20old%20style%20{{slapd.conf}}%285%29%20file%20to%20{{cn=config}}%20format][рекомендуется]] использовать *ldif* формат и преобразовывать
*slapd.conf* файл настройки с помощью утилиты *slaptest*. Конфигурация
*6/2.4* поставляется с тремя вариантами базовых настроек,
...
*slapd.d*. При этом, конфигурация с помощью *slapd.ldif.example*
подразумевает использование утилиты *slapcat*.

Такой переход между версиями и поддерживаемыми ими возможностями
порождает большое количество сочетаний способов настройки. Все они
являются взаимозаменяемыми и выбор того или иного из них отразится на
дополнительных параметрах, которые могут неявно появиться (или,
наоборот, отсутствовать) в файлах каталога *slapd.d*.

Например, если настраивать конфигурацию *5/2.3* с помощью конвертации
утилитой *slaptest* файла настройки *slapd.conf.obsolete*
(т.е. поставляемого с версией *2.4*, а не *2.3*!), а конфигурацию
*6/2.4* - при помощи каталога *slapd.d* по умолчанию, то в результате
каталоги *slapd.d* будут практически полностью совпадать (за
исключением несущественных полей). Но при этом надо иметь в виду, что
есть некоторые новые настройки в версии *2.4*, не в полной мере
поддерживаемые версией *2.3*, это касается правил доступа *olcAccess*
для пользователя *root*
(gidNumber=0+uidNumber=0,cn=peercred,cn=external,cn=auth).

Другой пример. Если настраивать конфигурацию *5/2.3* с помощью
конвертации утилитой *slaptest* файла настройки *slapd.conf* (теперь
уже "родного", поставляемого с версией *2.3*), а конфигурацию *6/2.4*
с помощью файла *slapd.ldif.example* и рекомендаций "быстрого
старта"[fn::http://www.openldap.org/doc/admin24/quickstart.html], то в
результате каталоги *slapd.d* будут очень сильно отличаться. Из-за
того, что "оффлайновая" утилита *slapadd* формирует рабочий каталог "с
нуля", множество параметров по умолчанию будет отсутствовать в таком
каталоге.
...
Для обеих конфигураций будем создавать каталог *slapd.d* с нуля при
помощи утилиты *slapadd*, используя *ldif* формат (то есть
*slapd.ldif.example* вариант базовой
настройки[fn::http://www.openldap.org/doc/admin24/quickstart.html]),
не применяя *.conf* файлы или готовый каталог *slapd.d*.
...
поскольку версия *2.4* официально поддерживает конфигурирование с
помощью *slapadd* и *ldif* формат. Для версии *2.3* официально
поддерживаемым является способ конфигурирования через *conf*
файл. Все дальнейшие изменения уже осуществляются с
помощью "онлайновых" утилит *ldapadd* и *ldapmodify*.
