...
Использование только *ldif* (см. раздел см. раздел [[#configure-slapd]]) формата позволяет единообразно
конфигурировать схему БД LDAP, начиная с корневой записи DIT (Directory
Information Tree) *cn=config* (см. раздел [[#minimal-schema]]) и заканчивая листьями (см. раздел [[#new-user-ldap]]) этого дерева ---
...
| *sudo.ldif*   | настройка организационной единицы *sudoers*, настроек по умолчанию для *sudo* и группы *wheel*, включенной в группу администраторов *sudo*     |
...
*userPassword*, *shadowLastChange* и *loginShell*: для владельца
записи --- write и для неавторизованного пользователя --- auth.
Анонимный доступ будет разрешен только для базовой записи подразделения
*people*. Используем учетную запись *authenticator* (см. раздел [[#client-commands]]) для ограничения
...
подразделения *people* и *groups*, а также пользователя
...
#+caption: *sudo.ldif*: настройка организационной единицы *sudoers*, настроек по умолчанию для *sudo* и группы *wheel*, включенной в группу администраторов *sudo*
#+name: sudo-ldif
...
Для проверки возможности аутентификации будем использовать запись (см. раздел [[#new-user-ldap]]) *username*. Протестируем подключение с помощью команды
#+begin_src sh
getent passwd username
#+end_src
