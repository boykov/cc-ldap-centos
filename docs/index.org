...
#+caption: *domain.ldif*: установка суффикса, менеджера базы *hdb* и его пароля, и ACL для базы *hdb*
#+name: domain-ldif
...
подразделения *people*, *groups* и *public*, а также пользователя
...
#+caption: *base.ldif*: настройка корневого суффикса, менеджера, организационных единиц и пользователя *authenticator*
#+name: base-ldif
#+INCLUDE: /home/eab/git/cc/cc-ldap-centos/schema/base.ldif src ldif

#+caption: *sudo.ldif*: настройка организационной единицы *sudoers*, добавление настроек по умолчанию для *sudo* и группы *wheel*, включенной в группу администраторов *sudo*
#+name: sudo-ldif
...
#+begin_src sh
ldappasswd -h $(ip) -x -D "uid=username,ou=people,dc=mercury,dc=febras,dc=net" -w p@ssw0rd -s new_p@ssw0rd
#+end_src

#+caption: Изменение пользователем значения *loginShell* с помощью утилиты *ldapmodify*, аутентификация проводится с использованием нового пароля
#+begin_src sh :exports code :noweb yes
<<modify-sh>>
#+end_src
...
наличии достаточных прав доступа, класс *shadowAccount*, поля
