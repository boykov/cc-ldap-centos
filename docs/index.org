...
|--------------------+--------------+----------------+-------------------------------|
| *Docker Engine*    | kairos       | *Ubuntu 14.04* | *Docker version 1.12.3*       |
|--------------------+--------------+----------------+-------------------------------|
...
| *Клиентский хост*  | gui          | *Debian 8*     | *phpLDAPadmin 1.2.3*, custom  |
...
Для запуска сервиса *ldap* (*slapd*) необходимо подготовить каталог и
...
Где работаем: контейнеры *server-.....*
#+end_verse
База данных LDAP содержит раздел *cn=schema*, куда
подключаются дополнительные специфические схемы данных. Подготовим
...
#+begin_note
В данном руководстве слово "схема" используется в выражении /схема
данных/, означающим файл с расширением *schema* или *ldif*, содержащий
описание классов и атрибутов. Также, оно используется в выражении
/схема базы данных LDAP/, т.е. для описания содержания, структуры и
ограничений целостности базы данных LDAP. В литературе выражение
/схема базы данных/ применительно к базе данных LDAP не встречается,
оно применяется для реляционных баз данных. Кроме того, /"база данных
LDAP"/ использует /базу данных *HDB*/ в качестве конечного хранилища
данных.
#+end_note
...
#+begin_src sh :exports results :results org
cat ../ldap-client/Dockerfile5 | grep openldap | sed "s|RUN ||g"
#+end_src
...
#+begin_src sh :exports results :results org
cat ../ldap-client/Dockerfile6 | grep openldap | sed "s|RUN ||g"
#+end_src
...
*pam_ldap* (см. также настроки для *authenticator* в подразделе
[[#minimal-schema]]).
...
Использование только *ldif* (см. подраздел [[#configure-slapd]])
формата позволяет единообразно конфигурировать схему БД LDAP, начиная
с корневой записи DIT (Directory Information Tree)
*cn=config* (см. подраздел [[#minimal-schema]]), корневой записи домена
(см. подраздел [[#root-domain]]) и заканчивая листьями (см. подразделы
...
| *sudoers.ldif*    | настройка организационной единицы *sudoers*, добавление настроек по умолчанию для *sudo* и группы *wheel*, включенной в группу администраторов *sudo* |
...
и атрибутов схемы базы данных LDAP.

#+name: tab:schema-examples
#+caption: Примеры схем, классов и атрибутов и их использования
| Схема                | Класс                         | Атрибут                  | Пример использования |
|----------------------+-------------------------------+--------------------------+----------------------|
| openldap hardcoded   | top                           | uid, cn                  | domain.ldif          |
...
| nis.schema           | posixAccount, posixGroup      | homeDirectory, gidNumber | user.ldif            |
| sudo.schema          | sudoRole                      | sudoUser                 | sudooers.ldif        |
...
** Установка схемы базы данных LDAP
:PROPERTIES:
...
правила доступа к атрибутам *userPassword* и *shadowLastChange*
(см. подраздел [[#new-user-ldap]]): для владельца записи --- /write/ и для
неавторизованного пользователя --- /auth/. Анонимный доступ будет разрешен только для
...
используем учетную запись *authenticator* (см. подраздел
[[#client-commands]]).

#+caption: *domain.ldif*: связывание суффикса, менеджера домена и его пароля с базой *HDB*; установка списка контроля доступа ACL
#+name: domain-ldif
...
записи дерева DIT средствами протокола *ldif* (эквивалентную настройку
...
Создадим подразделение *sudoers*, одну posix-группу *wheel* и
...
Полученные в подразделе [[#add-users]] в результате миграции учетные записи
пользователей кластера добавляются аналогично. Необходимая нам
...
# В подразделе [[#client-options]] было продемонстрировано расширение
