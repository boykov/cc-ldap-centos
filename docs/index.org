...
** Подготовка к конфигурированию схемы БД LDAP
:PROPERTIES:
...
#+begin_html
<br/><a onclick="toggleDiv2('myContent-manager',$(this));" class="fa fa-caret-down">/root/manager.ldif</a>
#+end_html
...
Дальнейшая настройка схемы осуществляется в разделе [[id:b64d7c36-b2b3-49ac-9620-d7c94eb9ea31][Схема БД LDAP]].
...
Файлы, которые были модифицированы [[id:6e7a8278-7334-46f7-b289-07da7f2b6d14][настройками]], в случае 5/2.3
...
изменения в схему. В частности, сконфигурируем базу *bdb* для
доменного имени *dc=mercury,dc=febras,dc=net*, пропишем пароль для
менеджера домена *cn=Manager,dc=mercury,dc=febras,dc=net*, добавим 3
подразделения *groups*, *people* и *sudoers*. Также, добавим в
*domain.ldif* правила доступа к атрибутам *userPassword* и
*shadowLastChange*: для менеджера домена и владельца записи --
write и неавторизованного пользователя -- auth.
...
** Добавление пользователя с помощью протокола LDAP
:PROPERTIES:
