...
*shadowLastChange*: владелец и *authentificator* имеют право на запись
/write/. Анонимный доступ будет разрешен только для заголовочной части
записи подразделения *public*, но не для ее содержимого. Для доступа к
записям пользователей и остальным атрибутам используем учетную запись
*authenticator* (см. подраздел [[#client-commands]]).
...
| *passwd*   | *Да*            |                                                                |
| *useradd*  | Нет             | *ldapadd*, атрибуты классов *posixAccount*, *shadowAccount*    |
| *usermod*  | Нет             | *ldapmodify*, атрибуты классов *posixAccount*, *shadowAccount* |
| *finger*   | *Да*            |                                                                |
| *chsh*     | Нет             | *ldapmodify*, атрибут *loginShell*                             |
| *chfn*     | Нет             | *ldapmodify*, атрибут *gecos*                                  |
| *.forward* | *Да*            | атрибут *mailRoutingAddress* класса *inetLocalMailRecipient*   |
