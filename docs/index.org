...
Конфигурация используемых в нашем docker-стенде машин:

| *Шлюз, docker host* | 172.17.0.1      |
| *Серверы каталогов* |                 |
|                     | cc-ldap-centos5 |
|                     | cc-ldap-centos6 |
| *Клиентские машины* |                 |
...
IP-адреса машин в подсети 172.17.0.x определяются на основе имен контейнеров.
...
Чтобы иметь доступ к серверу LDAP для конфигурирования схемы БД LDAP, внесем необходимые изменения: пропишем пароль менеджера схемы *cn=Manager,cn=config*.
...
Например, для конфигурации *5/2.3* требуется добавить настройки *frontend* базы, а для *6/2.4* эти настройки добавляются автоматически из исходного кода *OpenLDAP*.

Кроме того, добавим минимально необходимые настройки для *monitor* и *bdb*
...
#+caption: *slapd.ldif*: конфигурирование базы данных *bdb* и включение мониторинга баз данных
#+name: slapd-ldif
...
# #+begin_note
# Первичный доступ к схемам LDAP, использующим только *cn=config* формат
...
| *slapd.ldif*          | конфигурирование базы данных *bdb* и включение мониторинга баз данных |
| *front.ldif*          | добавление служебной базы данных *frontend* (только для *5/2.3*)      |
| *domain.ldif*         | установка суффикса, менеджера и ACL для *bdb*                         |
...
#+caption: *domain.ldif*: установка суффикса, менеджера и ACL для *bdb*
#+name: domain-ldif
