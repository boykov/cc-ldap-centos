...
#+begin_note
Команды и конфигурационные файлы прошли проверку на работоспособность в тестовом программном окружении, доступном в проекте cc-ldap-centos[fn::https://github.com/boykov/cc-ldap-centos].
#+end_note
...
предварительно сформированным каталогом *slapd.d*.

#+begin_note
Первичный доступ к схемам LDAP, использующим только предварительно
сформированный каталог *slapd.d* без *conf*-файла, осуществляется при
помощи *ldapi* интерфейса, когда в качестве *rootdn* выступает
...
<<schema2ldif>>
#+end_src
#+caption: *slapd.ldif*: формирование корневой записи DIT и контейнера для схем данных; конфигурирование базы данных *hdb* и подключение мониторинга баз данных
#+name: slapd-ldif
...
Дальнейшая настройка схемы осуществляется в разделе [[#schema]].
* Настройка аутентификации пользователей при помощи LDAP{{{fa(star-half-o)}}}
:PROPERTIES:
...
*pam_ldap* (см. также настроки для *authenticator* в разделе
[[#minimal-schema]]).
...
** Изменяемые файлы{{{fa(star)}}}
:PROPERTIES:
:CUSTOM_ID: client-files
:ID:       386e40de-d1f2-4dc8-994b-5870d4ec42fb
:END:

Файлы, которые были модифицированы настройками раздела [[#client-commands]] в случае *5/2.3*
...
который может работать как с локальными базами данных *files*, так и c
базой *ldap*.
...
#+caption: Опции, выставляемые в файлах для *5/2.3* (а)
#+INCLUDE: /home/eab/git/cc/cc-ldap-centos/gen/client5.diff src diff :lines "1-41"
...
#+INCLUDE: /home/eab/git/cc/cc-ldap-centos/gen/client5.diff src diff :lines "41-74"
...
#+caption: Опции, выставляемые в файлах *nslcd.conf*, *nsswitch.conf* и */etc/openldap/ldap.conf* для *6/2.4*
#+INCLUDE: /home/eab/git/cc/cc-ldap-centos/gen/client6.diff src diff :lines "1-38"
...
#+INCLUDE: /home/eab/git/cc/cc-ldap-centos/gen/client6.diff src diff :lines "38-"
...
Использование только *ldif* (см. раздел см. раздел [[#configure-slapd]])
формата позволяет единообразно конфигурировать схему БД LDAP, начиная
с корневой записи DIT (Directory Information Tree) *cn=config*
(см. раздел [[#minimal-schema]]) и заканчивая листьями (см. разделы
[[#new-user-ldap]], [[#add-users]]) этого дерева --- записями, содержащими
нужную информацию:
...
| *sudo.ldif*    | настройка организационной единицы *sudoers*, добавление настроек по умолчанию для *sudo* и группы *wheel*, включенной в группу администраторов *sudo* |
| *user.ldif*    | создание нового пользователя, принадлежащего к группе *wheel*                                                                                         |
...
стартовая часть этого ряда конфигурационных файлов
...
схема БД LDAP --- должна обеспечивать поддержку данной
функциональности. Нам потребуются два контейнера для хранения учетных
данных пользователей и групп. Разрешим доступ к данным пользователей
только для клиентских систем cо специальной учетной записью, запретим
анонимный доступ. Добавим класс учетных записей пользователей,
обладающих правами *sudo*.
...
изменения в схему. В частности, сконфигурируем (см. расп. [[domain-ldif]])
базу *hdb* для доменного имени *dc=mercury,dc=febras,dc=net*,
определим менеджера домена *cn=Manager,dc=mercury,dc=febras,dc=net* и
его пароль. Также, добавим в *domain.ldif* правила доступа к атрибутам
*userPassword* и *shadowLastChange*: для владельца записи --- /write/
и для неавторизованного пользователя --- /auth/. Значение атрибута
*loginShell* установим для любого пользователя системы --- /read/, а
для владельца --- /write/. Анонимный доступ будет разрешен только для
заголовочной части записи подразделения *public*, но не для ее
содержимого. Для доступа к записям пользователей и остальным атрибутам
используем учетную запись *authenticator* (см. раздел
[[#client-commands]]).
...
#+caption: *domain.ldif*: привязка суффикса, менеджера домена и его пароля, списка контроля доступа ACL к базе *hdb*
#+name: domain-ldif
#+INCLUDE: ../schema/domain.ldif src ldif

Далее, пропишем (см. расп. [[base-ldif]]) необходимые атрибуты, начиная с
корневого узла *dn: dc=mercury,dc=febras,dc=net*, добавим
подразделения *users*, *groups*, *system* и *public*, а также пользователя
*authenticator*. Создадим подразделение *sudoers*, одну posix-группу
*wheel* и сконфигурируем *sudoers* так, чтобы разрешить выполнение
*sudo* команд без ввода пароля для членов этой группы
(см. расп. [[sudo-ldif]]).

#+caption: *base.ldif*: создание суффикса, менеджера домена, организационных единиц и учетной записи *authenticator*
#+name: base-ldif
#+INCLUDE: /home/eab/git/cc/cc-ldap-centos/schema/base.ldif src ldif

#+caption: *sudo.ldif*: создание организационной единицы *sudoers*, добавление настроек по умолчанию для *sudo* и группы *wheel*, включенной в группу администраторов *sudo*
#+name: sudo-ldif
...
#+caption: *user.ldif*: создание нового пользователя, принадлежащего к группе *wheel*
#+INCLUDE: "../schema/user.ldif" src ldif

Полученные в разделе [[#add-users]] в результате миграции учетные записи
пользователей кластера добавляются аналогично. Необходимая нам функциональность
учетной записи пользователя определяется классами *posixAccount* и
*shadowAccount*.
...
(см. раздел [[#new-user-ldap]]) *username*. Протестируем подключение с
помощью команды
#+begin_src sh
getent passwd username
#+end_src
...
username:x:1050:1050:User Name:/home/webminder:/bin/bash
#+end_src
...
# В разделе [[#client-options]] было продемонстрировано расширение
# функциональности PAM: в дополнение к локальной базе данных *files*
# используется база данных *ldap*.
# Привычные системному администратору утилиты не могут работать с
# базой данных *ldap* напрямую. Хотя бы потому, что невозможно
# обеспечить взаимооднозначное соответствие между plain-текст файлами
# *passwd*, *group*, *shadow* и *sudoers* и записями в БД LDAP: как и
# в любой базе данных, записи могут быть вовлечены в более сложные
# связи между собой и недостаточно просто записывать эквивалент
# изменений, порождаемых этими утилитами, в БД LDAP.

#+caption: Изменение пользователем своего пароля с помощью утилиты *ldappasswd*
#+begin_src sh
ldappasswd -h $(ip) -x -D "uid=username,ou=users,dc=mercury,dc=febras,dc=net" -w p@ssw0rd -s new_p@ssw0rd
#+end_src

#+caption: Изменение пользователем значения *loginShell* с помощью утилиты *ldapmodify*
#+begin_src sh :exports code :noweb yes
<<modify-sh>>
#+end_src
...
sed -i "s|ou=People|ou=users|g" mercury.ldif
sed -i "s|ou=Group|ou=groups|g" mercury.ldif
#+end_src
