...
В CentOS 5 из базового репозитория устанавливается 2.3.43 версия OpenLDAP.
#+INCLUDE: "../ldap-server/Dockerfile" src dockerfile :lines "18-21"
#+begin_tip
Там, где это не вызывает недоразумений, вместо синтаксиса обычных команд иногда будет использоваться синтакс Dockerfile. Например, предыдущая команда означает просто src_sh[:exports code]{yum -y install openldap-server openldap-clients}.
#+end_tip
#+name: run-prefix
...
#+begin_src sh :tangle ~/git/cc/cc-ldap-centos/ldap-server/run.sh :exports none :comments noweb :noweb yes
<<run-prefix>>
<<run-db-config>>
<<run-slapd-conf>>
<<run-slapd-d>>
