...
echo binddn cn=authenticator,ou=system,dc=mercury,dc=febras,dc=net >> /etc/sudo-ldap.conf
echo bindpw secret >> /etc/sudo-ldap.conf

echo binddn cn=authenticator,ou=system,dc=mercury,dc=febras,dc=net >> /etc/pam_ldap.conf
echo bindpw secret >> /etc/pam_ldap.conf

echo binddn cn=authenticator,ou=system,dc=mercury,dc=febras,dc=net >> /etc/nslcd.conf
